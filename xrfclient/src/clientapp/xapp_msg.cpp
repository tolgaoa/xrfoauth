...
void xapp_msg::create_final_msg(std::string&str) {
...
unsigned char* xapp_msg::rsa_decrypt(unsigned char* cip_buf, long int cip_len){
...
void xapp_msg::prep_msg_2(unsigned char m_buf[], unsigned char sig_buf[], unsigned char msg_plain_1[], unsigned char msg_plain_2[]){
...
int xapp_msg::verify_sig(unsigned char* md_buf, unsigned char* sig_buf){

    EVP_PKEY *pubKey;
    pubKey = EVP_PKEY_new();
    FILE* fp = fopen("pub_xrf", "r");
...
int xapp_msg::final_verification(const std::string&rec_str, unsigned char m_buf[]){
    ...
    */
    ...
    prep_msg_2(m_buf, sig_buf, plain_buf_1, plain_buf_2);
    print_debug("\nmsg:\n",m_buf,RND_LENGTH);
    print_debug("\nsig:\n",sig_buf,RSA_SIG_LEN);
