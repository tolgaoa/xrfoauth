...
xpid=$(ps aux | grep xappoa | awk '{print $2}')

PROCESS_STAT=($(sed -E 's/\([^)]+\)/X/' "/proc/$xpid/stat"))
...
let PROCESS_USAGE_CLK="$PROCESS_UTIME + $PROCESS_STIME"
...
echo ${PROCESS_USAGE} | tee -a >> ./cpuusage.txt
