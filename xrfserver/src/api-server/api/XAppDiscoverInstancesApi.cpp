...
void XAppDiscoverInstancesApi::setupRoutes() {
    ...
    Routes::Get(*router, base + "/xapp/disc", Routes::bind(&XAppDiscoverInstancesApi::x_app_disc_inst_handler, this));
...
void XAppDiscoverInstancesApi::x_app_disc_inst_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    ...
    auto targetxAppServiceQuery = request.query().get("targetxAppService");
    Pistache::Optional<XAppService> targetxAppService;
    if(!targetxAppServiceQuery.isEmpty()){
        XAppService valueQuery_instance;
        if(fromStringValue(targetxAppServiceQuery.get(), valueQuery_instance)){
            targetxAppService = Pistache::Some(valueQuery_instance);
        }
    }
    auto reqestingxAppServiceQuery = request.query().get("reqestingxAppService");
    Pistache::Optional<XAppService> reqestingxAppService;
    if(!reqestingxAppServiceQuery.isEmpty()){
        XAppService valueQuery_instance;
        if(fromStringValue(reqestingxAppServiceQuery.get(), valueQuery_instance)){
            reqestingxAppService = Pistache::Some(valueQuery_instance);
        }
    }
    ...
    try {
        this->x_app_disc_inst(targetxAppService, reqestingxAppService, acceptEncoding, response);
