...
bool xrf_jwt::generate_signature(const std::string& xapp_consumer_id,
	...
	std::string priv_key;
	int kid; 
	generate_key_pair(jwks, priv_key, kid, pub_key);
	kidout = kid;
...
bool xrf_jwt::generate_key_pair(std::unordered_map<int, std::string>& jwks, std::string& priv_key, int& kid, std::string& pub_key){
